...
def train(ckpt_prefix, model, device, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            os.makedirs(rf"./res", exist_ok=True)
            best_checkpoint_path = rf"./res/{ckpt_prefix}_best_val_acc_{val_acc:.4f}.pth"
            torch.save({
                'epoch': epoch + 1,
            ...
            print(f"Saved new best model checkpoint at {best_checkpoint_path}")
        else:
            epochs_no_improve += 1  
    ...
    accuracy_plot_path = os.path.join("./res", f"{ckpt_prefix}_accuracy_plot.png")
    plt.savefig(accuracy_plot_path)
    print(f"Saved accuracy plot at {accuracy_plot_path}")

    with open(f"./res/{ckpt_prefix}_training_metrics.csv", mode="w", newline="") as file:
